...
//	    	exit( $header . $body2 . $footer );
    ...
    //接口访问日志
    public function visitlog($apptitle,$v,$interfacetitle,$visiturl)
    {
        $log = new VisitLog();
        $log->users_id =\Yii::$app->user->id;
        //var_dump(222);  exit;
        //$log->visit_log_id = 4;
        //$log->users_id = 2;
        $log->app_title = $apptitle;
        $log->interface_vision_title = $v;
        $log->interface_title = $interfacetitle;
        $log->visit_ip = \Yii::$app->getRequest()->getUserIP();
        $log->visit_url = $visiturl;
        $log->visit_time = time();
        $log->visit_accesstoken = 'sdfsdfa/\sdfs';
        if(!$log->insert()){
            var_dump($log->getErrors());
        }
//        $log->content = '【'.$log->realname.'】'. $content. '【'.$param1.'】'. $last;
//        $log->content = empty($param2) ? $log->content : $log->content.'【'.$param2.'】';
//        $log->ip = Yii::$app->getRequest()->getUserIP();
//        $isinsert = \Yii::$app->db->createCommand()->insert('visit_log',array('visit_accesstoken'=>time(),'visit_time'=>time(),'users_id'=>time(),'app_title'=>time(),'interface_vision_title'=>time(),'interface_title'=>time(),'visit_ip'=>time(),'visit_url'=>time()))->execute();
//        //var_dump($isinsert);exit;
//        if($isinsert){
//            //echo 222;
//        }else return $isinsert->getErrors();
    }
}